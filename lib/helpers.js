...
module.exports = self = {
  ...
  FILES: {
    LOCALIZED: {
      ...
      COUNTRY_MAP:  new File({
        path: '_core/country-map.js',
        url:  'https://raw.githubusercontent.com/umpirsky/country-list/master/country/icu/{localeSnake}/country.json'
      })
    },
    COMMON:    {
      LOCALES:      new File({
  ...
  grabLocalizedFiles: function (locale, root, onlyIfMissing) {
    ...
    vars.localeSnake = vars.locale.replace(/\-/g, '_');
  ...
  download: function (desc, onlyIfMissing) {
    var lib = require(desc.url.split(':').shift());
  ...
  cleanupKeySegment: function (segment) {
    return segment.replace(/(^|.)([A-Z]+)([^A-Z]|$)/g, function (dummy, before, str, after) {
      return (before || '') + '_' + str.toLowerCase() + (str.length > 1 ? '_' : '') + (after || '');
